(function( $ ) {
    ...
    AysPDPlugin.prototype.setEvents = function(e) {
        ...
        _this.$el.on('click', '.'+_this.htmlClassPrefix+'add_group_btn', function(e){
			...
			content +='<div class="ays-pd-group-close-button-div ays-pd-add-group-close-button-div"><input type="text" placeholder="Group name" class="ays-pd-group-name-inp"><span class="ays-pd-box-close-button">x</span></div>';
			content += '<div class="ays-pd-group-delete-button-block "><input type="button" value="'+aysPdLangObj.save+'" class="ays-pd-save_group_button"><a class="ays-pd-group-delete-button-item ">' + aysPersonalDictionaryAjaxPublic.icons.delete_icon+'</a></div>';
			content += '</div>';

			_this.$el.find('.'+_this.htmlClassPrefix+'save-groups-block').html(content);

			_this.$el.find('.'+_this.htmlClassPrefix+'save-groups-block').slideDown();
        });
        ...
        _this.$el.on('click','.ays-pd-group-tab .ays-pd_each_group_name',function(e){
            e.preventDefault();
            _this.$el.find('.'+_this.htmlClassPrefix+'save-groups-block').slideUp();
			var dataId = $(this).parents('.ays-pd-each_group_item').attr('data-id')
		...
		_this.$el.on('click','.'+_this.htmlClassPrefix+'previous-page-btn',function(e){
            ...
            _this.$el.find('.'+_this.htmlClassPrefix+'group-tab-add-layer').slideUp();
        });
        ...
        _this.$el.on('click','.'+_this.htmlClassPrefix+'each_word_item' + ',' + '.'+_this.htmlClassPrefix+'word-edit-btn', function(e){
			...
			content += '<div class="ays-pd-close-button-div ays-pd-word-translation-fields-parent"><input type="text" class="ays-pd-edit_word_field ays-pd-word_editing_fields" placeholder="'+aysPdLangObj.word+'"><span class="ays-pd-box-close-button">x</span></div>';
			...
			content += '<div class="ays-pd-word-delete-button-block "><input type="button" value="'+aysPdLangObj.save+'" class="ays-pd-edit-words-btn ays-pd-words_save_btn"><a class="ays-pd-word-delete-button-item ">' + aysPersonalDictionaryAjaxPublic.icons.delete_icon+'</a></div>';
			...
			$(this).parents('.'+_this.htmlClassPrefix+'words-box').next().slideToggle();
        });
        ...
        _this.$el.on('click','.'+_this.htmlClassPrefix+'group-edit-btn',function(e){
			...
			content += '<div class="ays-pd-add-group-form">';
			content +='<div class="ays-pd-close-button-div"><input type="text" placeholder="Group name" class="ays-pd-group-name-inp"><span class="ays-pd-box-close-button">x</span></div>';
			content += '<div class="ays-pd-group-delete-button-block "><input type="button" value="'+aysPdLangObj.save+'" class="ays-pd-save_group_button"><a class="ays-pd-group-delete-button-item ">' + aysPersonalDictionaryAjaxPublic.icons.delete_icon+'</a></div>';
			content += '</div>';
			...
			$(this).parents('.'+_this.htmlClassPrefix+'groups-box').next().slideToggle();
        });
        ...
        _this.$el.on('click','.ays_pd_add_word_button',function(e){
			...
			content += '<div class="ays-pd-close-button-div ays-pd-word-translation-fields-parent"><input type="text" class="ays-pd-word_field ays-pd-word_saving_fields" placeholder="Word"><span class="ays-pd-box-close-button">x</span></div>';
			...
			content += '<div class="ays-pd-word-delete-button-block"><input type="button" value="'+aysPdLangObj.saveAndClose+'" class="ays-pd-add-edit-words-btn ays-pd-words_save_and_close_btn"><a class="ays-pd-word-delete-button-item ">' + aysPersonalDictionaryAjaxPublic.icons.delete_icon+'</a></div>';

			
			_this.$el.find('.'+_this.htmlClassPrefix+'group-tab-words').html(content);

			_this.$el.find('.'+_this.htmlClassPrefix+'group-tab-words').slideDown();
		...
		_this.$el.on('click','.'+_this.htmlClassPrefix+'words_save_and_close_btn',function(e){
			...
			_this.$el.find('.'+_this.htmlClassPrefix+'group-tab-words').slideUp();
			var word = $('.'+_this.htmlClassPrefix+'word_field').val('');
			var translation = $('.'+_this.htmlClassPrefix+'translation_field').val('');
			$('.'+_this.htmlClassPrefix+'word_field').focus();
			_this.$el.find('.'+_this.htmlClassPrefix+'edit-words-btn').removeAttr('data-wordid');
    ...
    AysPDPlugin.prototype.selectWordsAjax = function(catId) {
        ...
        $.ajax({
            ...
            success: function(res) {
				...
				if(words[1] !== null){
					_this.$el.find('.'+_this.htmlClassPrefix+'header_title h3').html(words[1]);
				}
				var content = '';
				var content = '<div>';
